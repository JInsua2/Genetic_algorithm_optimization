...
def cruce(padre, madre):
    hijo = np.concatenate((padre[0:6], madre[6:12], padre[12:16]), axis=0)
    if (np.array(hijo).sum() < 205):
        hijo = generar_greedy(hijo)
    hija = np.concatenate((madre[0:6], padre[6:12], padre[12:16]), axis=0)
    if (np.array(hija).sum() < 205):
        hija = generar_greedy(hija)
    return hijo, hija
